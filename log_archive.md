...
- **What it was (Sep 2–4 2026, issue #97, branch `jesse`, 39 commits, 292 files, +12.6k/−14.0k lines; Rust alone 239 files, +9.1k/−9.1k):** a DRY/KISS/SRP sweep of the engine run through the three-model handoff loop — Claude planned and committed, gemini typed each batch from a handoff prompt, kimi and Claude reviewed every diff, Jesse adjudicated. Ten batches: correctness fixes with regression tests; dead-API deletion and workspace hygiene; the comment/naming/`#[allow]` sweep; DRY in the systems crates, in engine_core, in the editor commands and command API, and in the editor UI; SRP splits on the engine side (loader and serializer one function per variant, a golden scene fixture, the frame tail as named phases) and the editor side (`EditorGame::update` as named phases, one function per API verb, play transitions in their own module, headless seams for the `GameContext` takers); the test suite cut to a keep-list per crate before any behaviour change in that crate; and the docs — File Maps that no longer restate the module tree, a "Pitfalls and their guard tests" table per crate guide, the nine `ANALYSIS.md` files deleted after their forty live lines moved. The plan and the reviewer ledger: `coordination/cleanup-2026-09/`; the keep-lists, audits and design sketches were pruned after the merge and read as `git show cleanup-2026-09-records:coordination/cleanup-2026-09/<file>`.
- **Reviewer ledger (49 rows):** kimi 33 reviews, 148 findings, 124 real, 7 false, 17 policy rebuts, about 11 minutes each; gemini 5 plan-mode reviews, 28 findings, 28 real, about 7 minutes; Claude 11 code reviews, 55 findings, 50 real. Every kimi code review verified its claims by running the suite and grepping callers; its false positives were all plan-level. Gemini as a plan reviewer caught what the builder sees (a test the section's own edit would break, an ordering that could not compile, a gate that could never pass). The default-reviewer ruling from these numbers is Jesse's.
- **Lessons, in the order they cost something:** the executor's report is not a description of the tree — batch 8 reported a section done that had not landed, batch 9 named variants and fields that did not exist, batch 10 marked a guarded pitfall unguarded; the planner greps every symbol and test the report names before review. A correction to a batch goes into the batch section the executor reads, never a side note (batch 2's missed list). Re-verify the section against the tree before every handoff: batch 3's named a file batch 2 had deleted. A plan section that is the whole spec, with today's line numbers, beats a summary pointing at design docs the tree has outrun (batches 7–10). The review hook's threshold counts Markdown, so a docs batch is reviewed like any other. Keep-list cuts land before behaviour changes in the same crate, so a deleted test that would have failed is caught in review, not lost. The safe landing form is the bare `ADV_REVIEWED=1 git -C <repo> … -F <msg> --pathspec-from-file=<spec>` as its own command; a leading `cd` or a heredoc in the same command makes the hook miss the prefix. One batch out at a time; while it is out the planner neither edits nor runs cargo in that checkout.
