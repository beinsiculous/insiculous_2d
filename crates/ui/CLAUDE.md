...
- `interaction/` — widget state and persistent focus; `wants_mouse()` indicates a widget owns the current press→release gesture. Blocking regions carry the `UiLayer` that claimed them: a widget in an overlay scope is inert only under a region from a higher layer, so a modal's scrim reaches a strip or a dropdown opened after it while a dropdown over the strip stays live.
