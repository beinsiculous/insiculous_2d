...
- **The toolbar strip**: `render_toolbar_and_play_controls` lays the tools and the play controls out with `editor::toolbar_strip::layout` and renders them inside the strip's scope, then draws the shed tools' overflow menu after that scope closes (overlay scopes cannot nest). A panel shown as the dock's narrow-mode overlay has its content rendered inside a floating-band scope in `panel_renderer::render_panel_content`, or the overlay's own blocking rect would make the field it exists to expose inert.
