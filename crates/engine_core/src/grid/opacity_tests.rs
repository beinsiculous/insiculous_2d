...
fn resting_grid_is_more_transparent_than_moving_grid_and_settles_back_on_both_lattices() {
    for (name, mut g) in [
        ("hex", GridMesh::new(6, 5, 10.0, Vec2::ZERO).with_damping(0.2)),
        ("square", GridMesh::new_square(5, 4, 10.0, Vec2::ZERO).with_damping(0.2)),
    ] {
...
fn activity_alpha_never_exceeds_color_alpha_lingers_after_the_rise_and_fraction_one_is_uniform() {
    ...
    assert!(max_alpha(&mut g) <= g.color.w + 1e-6, "activity must never push alpha past color.w");
