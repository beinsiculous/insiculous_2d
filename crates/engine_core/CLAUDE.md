...
- `game/frame_tail.rs` — post-update tail: particles, then `step_world_systems` — `ecs::SpriteAnimationSystem`, `ecs::LifetimeSystem`, `ecs::ClipStateMachineSystem`, in that order — on `delta_time * time_scale` so pausing freezes all of it. Every game gets lifetimes and clip machines from here; a game must not own a `LifetimeSystem` of its own (it would halve every lifetime).
