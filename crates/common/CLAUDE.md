...
- `vfs.rs` — asset and scene read/write/list seam (`read`, `write`, `list_files`, `remove_prefix`); canonical key is joined path string `{asset_base}/{relative entry}`, with `list_dir_files` and `list_files` sorted on both native and wasm.
...
| Scene and asset writes must go through `common::vfs::write` / `write_string` so parent directories are created natively and writes land in `MemFs` on wasm | `src/vfs.rs test_mem_fs_write_and_read_round_trip` |
