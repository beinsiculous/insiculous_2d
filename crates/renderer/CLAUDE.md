...
| On the web a `Resized` can carry the size the window was asked for while the page's stylesheet holds the canvas box smaller, and the observer that would correct it never fires because the box never changed; the engine's `Resized` handler settles the size against `window.rs::shown_size` (the client box × device pixel ratio — the embed contract keeps the canvas free of padding and border, so it is the content box winit measures) in `resize_for_shown_box`: the editor's surface follows the box (`GameConfig::surface_follows_web_box`), a game's keeps its configured size and the page scales the canvas, with the pointer scaled from the box to the surface — a fixed 800×600 world shown through a phone's 324×243 box was a centre crop with both paddles outside it; the boot's forcing resize is the one call that must keep the configured size, or the 1×1 pre-layout trap returns, and it settles against the box in a second step | — none; the playground's hit probe, the Tong page at a phone width |
| Winit focuses the canvas on every pointer press with a plain `focus()`, which scrolls a partly visible canvas into view between press and release, so every hit lands above the cursor by the scroll; `focus_before_winit_does` focuses first with `preventScroll` from a capture-phase listener, and any other focus call on the canvas must pass `focus_without_scroll()`; the boot focus does not scroll either, so a page that wants its canvas in view on load anchors it itself | — none; the playground's hit probe |
| A drag that starts on the page and crosses into the canvas fires no `pointerdown` there, so the canvas stays unfocused until some later press scrolls mid-gesture; the same listener's `pointermove` half focuses a canvas a held pointer moves over, and bails while an input, textarea, select, button, summary, dialog or contenteditable holds focus, or while the page has a non-collapsed text selection, so drag-selecting text keeps its selection | — none; the playground's hit probe |
