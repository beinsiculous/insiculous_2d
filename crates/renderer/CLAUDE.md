...
| On the web a `Resized` can carry the size the window was asked for while the page's stylesheet holds the canvas box smaller, and the observer that would correct it never fires because the box never changed; the engine's `Resized` handler takes `window.rs::shown_size` (the client box × device pixel ratio — the embed contract keeps the canvas free of padding and border, so it is the content box winit measures) for the surface, the camera viewport and the tracked size alike through one `resize_everything`, the game's `on_resize` included — the boot's forcing resize is the one call that must keep the configured size, or the 1×1 pre-layout trap returns, and it follows the box in a second step | — none; the playground's hit probe |
