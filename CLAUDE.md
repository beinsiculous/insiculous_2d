...
- **ECS**: HashMap-based per-type storage, type-safe queries, data-driven UI components (UiLabel/UiPanel/UiButton + UiAnchor), named-clip `SpriteAnimation` over `SheetGrid` (`play`/`ensure_playing` by clip name; `SpriteAnimationSystem` writes `Sprite.tex_region`)
- **Renderer**: WGPU 28.0.0, instanced sprites, SDF shapes (rounded rects/circles/borders in the sprite shader), nearest/linear `TextureFilter`, device-loss fail-stop (`DeviceLossLatch` in `device_status.rs` — wgpu lost/uncaptured-error callbacks, every render-path entry guards; same-size/zero-size surface reconfigures deduped)
- **Physics**: Rapier2d integration, presets, editor-facing enum helpers (`RigidBodyType::ALL/label/index`, `ColliderShape` variant cycling with dimension carry-across)
- **UI**: Immediate-mode, `UiLayer` z-bands (per-layer draw collection flushed in enum order at end_frame — Content/PanelChrome/Floating/Modal/Tooltip/DragGhost; `begin_overlay` = Floating sugar), fontdue integration, real text editing (cursor/selection/key-repeat; numeric `float_input` with drag-scrub/arrow-nudge/soft-range + red invalid state (`FloatFieldOpts`/`FloatInputResult`) + free-form `text_input`, full A–Z/space typing; `focus_text_input` programmatic focus; `take_edit_commit` gesture flag), hover tooltips (`UIContext::tooltip(anchor, text)` on a rest-to-show timer, one per frame, drawn on `UiLayer::Tooltip`), the pointer-shape request (`CursorIcon` + `request_cursor`/`requested_cursor`, mapped to the platform by the host), Image draw command
- **Input**: Event-based, generic action mapping (`InputMapping<A>`) + player-aware `InputSettings` layer (P1/P2 device routing, axis-as-button, serde bindings) + gilrs hardware backend in engine_core (GAP-001 closed Jul 2026)
- **Audio**: Rodio backend, `manager/{mod,music,tests}.rs` split (device/SFX vs music/buses), gesture-gated web upgrade (`enable_output()` + pending music; the engine calls it on the first activation gesture — H7 closed Aug 27 2026) (spatial audio components exist in ecs but have no runtime system yet)
- **Engine Core**: Game API, managers, scene serializer, generic pickups, shared arcade scaffolding (`MenuInput`, `spawn_background`, `default_playfield_grid`, `RENDER_UNIT`), tilemap render pass, main-camera sync, input-settings JSON persistence (save-on-change dirty tracking), runtime window retitling (`ctx.set_window_title` request + `WindowManager::set_title`), post-tonemap UI render pass (UI draws to the swapchain AFTER bloom/tonemap — authored UI colors display exactly; game/UI batches stay separate end-to-end), `save_store` persistence seam (native atomic files / wasm localStorage + `insiculous-save` event — `docs/WEB_SAVES.md` is the site contract), `Scores` high-score facility (`ctx.scores`), gilrs gamepad backend, PauseMenu + MenuPanel chrome (localizable via `PauseMenuLabels`), localization (`Strings`, RON locale files, per-locale fonts), data-driven UI element pass (`UiButtonPressed` events), input-driven camera look-ahead (`CameraFollow`), texture-filter config knob, sprite-sheet pipeline (`load_sprite_sheet`, `.sheet.ron` sidecar schema in `sheet_file.rs`, sidecar-as-SSOT scene reload, sidecar-declared filter on scene texture refs, E5 tex_region/visible/#solid:RRGGBB scene round-trip), GPU device-loss fail-stop (RenderManager surface-error streak escalation + `GameRunner.render_fatal` — the frame loop STOPS on device loss instead of submitting to a dead queue, which crashed Firefox's in-process WebGPU; web shows "reload the page", native takes clean shutdown)
- **Editor**: Dockable panels (hide/collapse/resize, View-menu toggles with check marks and toolbar strip toggle buttons, persisted layout), viewport, inspector (incl. string fields + UI components), hierarchy, asset browser + drag-drop state, typography/theme tokens (WCAG surface ladder surface_0..4 + popup_border with luminance guard tests), command API Stages A+B (`command_api/` — list/describe/selection/scene/commands queries + set/add/remove/rename/create/delete/select/undo/redo/save/batch write verbs, ALL through CommandHistory so API mutations are GUI-undoable; name-first EntityRef, transport-agnostic dispatch, generic `SetComponentValueCommand` over `StoredComponent`, non-finite rejection + GUI-parity hard floors), CommandHistory dirty watermark (id-of-top; merges reassign ids + clear redo), crate-shipped DejaVu chrome fonts (regular/bold/mono via include_bytes), shared `ScrollState` (inspector/hierarchy/assets), width-aware inspector layout (`row_layout.rs` pure layout math — measured/ellipsized labels, composite Vec2/RGBA rows in `composite_rows.rs`, right-aligned remove [X]), editable entity Name (registry builtin + `SetNameCommand`; F2 inline rename in the hierarchy via `focus_text_input`, unnamed entities materialize a Name only on non-empty commit, duplicate-name status-bar warning on BOTH rename paths), fully editable Behavior strings + EntityTag (string_edit) and RigidBodyType/ColliderShape cycle rows (the audit's fake '  Type:/  Shape:' sub-headers are gone — they're real fields now), numeric-field UX (#30: drag-scrub + arrows + soft ranges — friction 2.0 finally DISPLAYS as 2.00, `edit_normalized_f32` deleted; degrees with ° wrap on rotations; `CommandHistory::break_merge` gesture boundary), drawn authoring grid (#36: pure `grid_segments` LOD/axes + `render_grid_overlay` clipped ui.lines — the sprite pipeline with zero callers is gone), fixed gizmos (#38: default tool Move, annulus rotate ring with click-through center, multiplicative per-axis scale, panel clipping, multi-root drags committing ONE undo entry, Escape cancel with polled latch), reachable non-residual-eating snapping (#37: View-menu toggle + S + hold-Ctrl, primary-anchor snapped delta shared across roots, zero-grid NaN guard), visible marquee (#39: live fill+border rect, (0,0) sentinel dead, Ctrl toggles/Shift adds, Escape cancels incl. sub-threshold presses; `SelectionRect` deleted), ONE chord-aware shortcut system (#40: editor-owned `EditorBinding` Chord/KeyAnyMods/Mouse + `resolve` with exact-chord-wins and full-tuple eviction, single `dispatch_editor_action`, arrow-nudge as merging `NudgeCommand` = one undo per key hold, Escape cascade, Ctrl+A, entity clipboard `clipboard.rs` — `SpawnTreeCommand`/`DeleteTreeCommand` with whole-subtree undo and GPP-14 id-stable redo; Duplicate/Cut refactored onto them); the editor also runs in the browser at `/playground/` on wasm32/WebGPU (the `playground` crate: IndexedDB project store, JS bridge, zip export/import)
- **Editor Integration**: `run_game_with_editor()` wrapper + inspector writeback + play/pause/stop + scene save/load + viewport↔render camera sync + asset browser panel + editor prefs persistence + editor-font scoping (locale fonts apply to the game view only) + engine-time freeze outside Play (particles/animations hold still while Editing) + play-session data-loss guards (save/new/open refused mid-simulation, snapshot loss warnings on Play and Stop; load_scene parses + dry-runs into a scratch World before touching the live one), --api stdin/stdout query+write transport (hosted create/save verbs — factories + save choke point; `ApiSession` batch committed on Play, discarded on Stop), OS-title dirty indicator, unified key dispatch (`handle_editor_key` → `EditorInputMapping::resolve` → `dispatch_editor_action`; polled Q/W/E/R path deleted) + entity clipboard handlers + multi-root gizmo drag state (`gizmo_drag.rs` — idempotent apply, one-entry commit, Escape restore), `ProjectHost` (data-only host for a data project — scene + `Scripts`, no game crate — which owns the physics step and steps the script runner)

### Key Metrics
- **Tests**: `cargo test --workspace` is the truth; the invariant is 0 failed, 0 ignored. No doc records a count (Sep 2026 ruling)
...
- Games (in `../games/`): breakout, pong, space_invaders, snake, asteroids, frogger — all tested, all clippy-clean, all 2-player; Pong and Frogger are fully localized (en + pirate, locale-driven font); Frogger is the first Tilemap consumer (Jul 2026)

### Current Priority
**The Deion Pivot** drives the roadmap (see `PROJECT_ROADMAP.md`, reworked Jul 28 2026 via adversarial review): the project's identity is **Deion the Insiculous** (SNES-styled hero — ball of DEIONized water with an icicle mohawk — in a food-coded world); the geometry-wars neon look becomes an FX/accent layer. The 20 Games Challenge is **paused at game 7 (Tetris)** while Phases E–I land: **E** engine asset pipeline (nearest filtering, `SheetGrid`, `SpriteAnimation` rework into named clips that actually write `Sprite.tex_region` — today it's disconnected from rendering, `.sheet.ron` sheet import, scene round-trip fixes), **F** `../deion_assets/DEION_STYLE.md` + asset production (Astra draws the sheets and tiles in Aseprite, Jesse draws the concepts and does the cleanup pass, agents do sidecars; 16px cells × 5 = RENDER_UNIT), **G** re-skin all 6 games (Pong → Frogger → Breakout → Snake → Invaders → Asteroids), **H** WASM port (H1 rodio-audio spike first; fetch-by-default assets; WebGPU-only), **I** deploy (Cloudflare Workers site `../insiculous_web/` — owned by Mily's GitHub account `milyramic` since Aug 2026 — then free itch.io via butler; Steam checklist only), **J** Mobile and the stores (outline only, rewritten Sep 25 2026: mobile web, native builds, the six games as free store apps with their AI art, then Insiculous Arcade — the first paid release, hand-drawn art only). **Studio premise (Aug 19 2026): Be Insiculous is an AI dev studio** — AI-assisted development is the product story; the **tiered AI-art rule** replaces "AI art never ships": free releases (website, free itch.io, free store apps) may ship AI art as workflow showcase, paid releases never do (`check_no_ai_assets.sh` gates paid paths only; DEION_STYLE.md §6 is SSOT). E1 (`TextureFilter` knob), E2 (`common::SheetGrid`), E6 (atlas.rs deleted) shipped Jul 30 2026 via adversarial-reviewed batch; H1 WASM spike PASSED same day (stay on rodio; see `coordination/H1_SPIKE.md`, listen test PASSED by Jesse — rodio decision FINAL). E3 (named-clip `SpriteAnimation`, wired to rendering + pause + editor freeze) and E4 (`load_sprite_sheet` + `.sheet.ron` v1 schema in `sheet_file.rs`) shipped Jul 30 2026 via the settled `review/plan-v4.md` — **SCHEMA FREEZE REACHED**. **Aug 19 2026: the Pong web vertical slice SHIPPED** — engine runs on wasm32/WebGPU (`common::clock` + `common::vfs` + web boot fetch, cfg-split frame loop with the native path byte-identical, async renderer init, `renderer::insert_canvas_into_dom`), Pong builds at 2.5 MiB via `scripts/build_wasm.sh`, browser-verified interactive, and the site deploy is staged in `../insiculous_web/` awaiting Jesse's push (I1). **Phase H is COMPLETE (Aug 27 2026)**: H6 save_store + web-saves wave (#6/#17), H7 gesture-gated audio (#8 — `enable_output` + first-gesture hook, pong paddle beep is the demo consumer, browser-verified), H8 wasm CI guard (#7 — `scripts/check_wasm.sh` + `wasm-check.yml`, workspace-scope, arc-lint decided), H5 include_bytes remainder closed won't-do (#9). **Editor Sprint 2 COMPLETE (Aug 27 2026)** — #50 (scene-load data-loss guard incl. AssetManager (path,filter) texture-load dedupe), #24 (CommandHistory dirty watermark + OS-title indicator; the confirm dialog on the Modal layer shipped later), #25 Stage A + #33 Stage B (command API queries + writes, `docs/EDITOR_COMMAND_API.md`, Stage C–D open), #29 (UiLayer z-bands), #28 (shared ScrollState + window-anchored add-component popup on Floating), #27 (crate-shipped DejaVu chrome fonts), #26 (post-tonemap UI pass — the audit's "gamma bug" was the Reinhard tonemap over UI, NOT an sRGB double-encode; WCAG ≥1.35:1 surface ladder + ≥3:1 popup borders, guard-tested). All seven kimi-reviewed; screenshot pass deferred (screen locked during the session). **Editor Sprint 6 ("polish & riders", #46/#51/#54/#55/#66) COMPLETE Sep 1 2026** — every item kimi-reviewed (one plan round + one code round per issue): #66 set/add refuse unissued texture handles, #55 soft-range typed values warn on the status bar, #54 numeric fields in the mono face, #51 primary-selection affordances (hierarchy fills + accent, inspector heading, collider widths, Shift range select), #46 **`GridBackdrop`** — the playfield spring grid as scene data (`ecs::GridBackdrop` + `engine_core::grid::GridBackdropSystem`, normalized rebuilds, `translate` on move, `ripple` impulse channel, `GridBackdropReset` on Stop; the six games' migration is the follow-up issue). **Editor Sprints 3 (#30–#35) AND 4 (#36–#40, "direct manipulation") COMPLETE Aug 28 2026** — all kimi-reviewed (sprint 4: 2 plan rounds + 4 code rounds; follow-up #59 selection-restore-on-undo). **The Web Playground SHIPPED Sep 6 2026** (#48, #49; ten batches, Sep 4–6; `docs/WEB_PLAYGROUND.md`, `docs/SCRIPTING.md`, `docs/WEB_SAVES.md`; the template `beinsiculous/game-template`). **The Playground UX sprint SHIPPED Sep 12 2026** (thirteen batches, Sep 8–12, a handoff loop reviewed by kimi and codex throughout — `coordination/playground-ux/plan.md`, `reviewer-comparison.md` beside it): Stop keeps or discards paused edits, the toolbar strip, the asset browser, the game-only preview, the site's application shell and the `Play ↗` preview window, the first run, the inspector during Play with collapsible sections and the colour editor, quieter overlays / View toggles / tooltips / resizing / Reset Layout, save state, and browser usability plus a performance budget — the focus ring, Tab/Shift-Tab traversal, an idle throttle, and a hard bundle-size gate, `docs/WEB_PLAYGROUND.md` § Acceptance and § Budget. Follow-ups: insiculous_2d#144 (keyboard-only entry into the inspector, scroll-into-view for an off-panel traversal target). Next actionable: the Studio Board — no editor sprint is queued yet as of this writing (`gh issue list -R beinsiculous/insiculous_2d`); F/G asset production continues as the parallel art track (F1 style guide exists — `../deion_assets/DEION_STYLE.md`), plus engine leftovers E5 (gated on F3 for the `#rgba` error) and E7 (alpha-cutoff). Editor: Phase 1 complete; the old Phase-2 (Ideal Editor UI) lettering is retired, and `docs/EDITOR_UX_AUDIT.md` is history (reconciled 2026-09-10, every item marked in the file; its §7 ran as sprints 1–5 and sprint 6, superseded by Playground UX above).
**History**: Phase A (games 1–5) ☑, Phase B (CameraFollow/Lifetime/Tilemap) ☑, game 6 Frogger ☑ (`../games/frogger/`, first Tilemap consumer).
...
- Open debt = Studio Board issues with the `tech-debt` label
  ...
  on the game's own repo). The per-crate `TECH_DEBT.md` and `ANALYSIS.md` files are
  retired — never recreate one; file/update an issue instead. Each crate guide's
  "Pitfalls and their guard tests" table is the index of the test that fails when a
  known trap regresses: a renamed test updates its row, and `— none` means the trap is
  unguarded, not that the row is stale.
...
cargo test --workspace               # Run every test (0 failed, 0 ignored is the invariant)
...
**Test Status:** run `cargo test --workspace`; the only acceptable summary is 0 failed, 0 ignored.
